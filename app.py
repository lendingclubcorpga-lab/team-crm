...
with tab2:
    ...
    if not st.session_state["is_admin"]:
        ...
        if uploaded_file is not None:
            ...
            # 🔄 NEW ENGINE: Automatically read spreadsheet rows if it is a CSV or Excel file
            is_spreadsheet = file_extension in ["csv", "xlsx"]
            df_to_import = None
            
            if is_spreadsheet:
                try:
                    # Reset stream position and read into a pandas DataFrame
            ...
            if st.button("Commit File & Process Registry Matrix"):
                try:
                    with conn.session as session:
                        # Step A: Save the raw backup file into crm_files table
                        ...
                        # Step B: If it's a spreadsheet, unpack rows and inject into searchable client table
                        rows_inserted = 0
